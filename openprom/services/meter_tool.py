...
_COMMON_CHAR_RANK: Dict[str, int] = {
    # 平声常用韵字（部分示例，可按需扩展）
    "一": 1, "天": 1, "烟": 2, "然": 2, "山": 1, "边": 2, "间": 1, "年": 1,
    "前": 1, "春": 1, "风": 1, "中": 1, "空": 1, "红": 1, "东": 2, "同": 3,
    "江": 1, "长": 1, "芳": 2, "阳": 1, "乡": 2, "堂": 2, "香": 1, "光": 1,
    "霜": 2, "窗": 2, "凉": 2, "声": 1, "城": 2, "明": 2, "情": 2, "行": 2,
}
...
def _detect_couplet_violations(upper: str, lower: str) -> Tuple[bool, List[str], List[Dict[str, Any]]]:
    ...
    for i in key_pos:
        if u_seq[i] * l_seq[i] != -1:
            ...
            details.append({"pos": i, "rule": "二四六分明", "upper_char": upper[i], "upper_tone": u_seq[i], "lower_char": lower[i], "lower_tone": l_seq[i]})

    # 仄起平落
    if u_seq[-1] != -1:
        violations.append(f"上联尾字“{upper[-1]}”非仄声")
        details.append({"pos": len(upper)-1, "rule": "上联尾字仄声", "char": upper[-1], "tone": u_seq[-1]})
    if l_seq[-1] != 1:
        violations.append(f"下联尾字“{lower[-1]}”非平声")
        details.append({"pos": len(lower)-1, "rule": "下联尾字平声", "char": lower[-1], "tone": l_seq[-1]})
    ...
    for name, tones, text_line in [("上联", u_seq, upper), ("下联", l_seq, lower)]:
        if len(tones) >= 3:
            ...
            if len(definite) >= 2 and all(t < 0 for t in definite):
                ...
                details.append({"rule": "三仄尾", "line": name, "chars": text_line[-3:], "tones": last3})
            if len(definite) >= 2 and all(t > 0 for t in definite):
                violations.append(f"{name}三平尾")
                details.append({"rule": "三平尾", "line": name, "chars": text_line[-3:], "tones": last3})

    return len(violations) == 0, violations, details


def check_meter(text: str, meter_type: str, pattern_name: Optional[str] = None, strict: bool = False) -> Dict[str, Any]:
    ...
    if meter_type == "couplet":
        ...
        result["pingze_sequence"] = [get_sequence(upper), get_sequence(lower)] if upper and lower else []
...
def explain_rule(rule: str) -> str:
    ...
    explanations = {
        ...
        "rhyme": (
            "押韵：律诗偶数句（二四六八句）及首句可入韵时须押同一韵部，多用平声韵。"
        ),
