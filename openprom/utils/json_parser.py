...
def _extract_json_string(content: str) -> Optional[str]:
    ...
    # 策略2：markdown代码块 - 无标记
    code_block_match_generic = re.search(r"```\s*\n?(.*?)\n?\s*```", content, re.DOTALL)
...
def validate_and_fill_defaults(
    data: Dict[str, Any], required_fields: Dict[str, Any]
) -> Dict[str, Any]:
    """验证并填充默认值
...
def parse_llm_json_response(
    content: str, required_fields: Optional[Dict[str, Any]] = None, max_repair_attempts: int = 3
) -> Dict[str, Any]:
    """解析LLM返回的JSON响应（增强版）
    ...
    if json_str is None:
        raise JSONParseError(f"无法从响应中提取JSON。响应长度: {len(content)}", content)
