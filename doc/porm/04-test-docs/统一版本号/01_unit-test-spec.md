...
|--------|----------|----------|----------|----------|--------|
| TC-001 | 版本号统一 | import porm | `porm.__version__` | 返回 "4.2.0" | P0 |
...
| TC-006 | 弃用代码清除 | import porm | `import porm` | 不崩溃, 无 PormTUI/launch_tui | P1 |
| TC-007 | YAML 提示文件解析 | yaml.safe_load | load all prompts/*.yaml | 全部通过 | P0 |
| TC-008 | ruff lint | ruff check | `ruff check porm/` | All checks passed | P0 |
