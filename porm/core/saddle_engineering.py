...
class ProcessController:
    ...
    def _register_defaults(self):
        ...
        self._interventions.append(self._validate_consistency)
    ...
    def _normalize_weights(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'details' in result and isinstance(result['details'], dict):
            ...
            if scores:
                ...
                # 如果主分数与加权平均差异过大，进行调整
                if 'score' in result:
                    diff = abs(result['score'] - avg_score)
                    if diff > 20:  # 差异超过20分
                        # 使用加权平均作为参考，但保留原始分数的权重
                        result['score'] = round(0.7 * result['score'] + 0.3 * avg_score)
    ...
    def _validate_consistency(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'score' in result and 'reason' in result:
            ...
            # 高分应该有正面评价
            if score >= 80 and any(word in reason for word in ['差', '不好', '问题', '不足']):
                inconsistencies.append("高分但评价负面")
            
            # 低分应该有负面评价
            if score <= 40 and any(word in reason for word in ['优秀', '出色', '完美', '典范']):
                inconsistencies.append("低分但评价正面")
        
        if inconsistencies:
            result['_inconsistencies'] = inconsistencies
...
class OutputController:
    ...
    def _apply_nlp_correction(self, context: ControlContext) -> ControlContext:
        ...
        if 'score' in llm_result:
            llm_score = llm_result['score'] / 100.0
    ...
    def _apply_meter_correction(self, context: ControlContext) -> ControlContext:
        ...
        if not meter_analysis.get('is_valid', True):
            # 格律违规，降低分数
            if 'score' in llm_result:
                original_score = llm_result['score']
                # 根据违规严重程度降分
                error_count = len(meter_analysis.get('errors', []))
                penalty = min(error_count * 5, 30)  # 每个错误扣5分，最多30分
                llm_result['score'] = max(0, original_score - penalty)
                llm_result['_meter_penalty_applied'] = True
                llm_result['_meter_penalty'] = penalty
                
                context.decision_chain.append(
                    f"格律修正: {original_score} -> {llm_result['score']} (违规扣{penalty}分)"
                )
...
class SaddleEngineering:
    ...
    def __init__(self):
        ...
        # 控制策略配置
        self._strict_mode = True  # 严格模式
        self._max_violations = 0  # 允许的最大违规数
    ...
    def execute(
        ...
        # 2. 过程层控制
        context.llm_parsed_result = self.process_controller.intervene(
            context.llm_parsed_result
        )
        ...
        if self._strict_mode and len(final_violations) > self._max_violations:
            ...
            # 计算最终分数
            context.final_score = context.llm_parsed_result.get('score', 0)
