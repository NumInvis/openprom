...
def _extract_json_string(content: str) -> Optional[str]:
    ...
    # 策略3：智能括号匹配 - 找到最外层的完整JSON对象
    # 使用栈来正确处理嵌套的 {} 和 []
    brace_stack = []
    start_pos = -1
    
    for i, char in enumerate(content):
        if char == '{' and not brace_stack:
            start_pos = i
        if char in '{[':
            brace_stack.append(char)
        elif char in '}]':
            if brace_stack:
                expected = '}' if brace_stack[-1] == '{' else ']'
                if char == expected:
                    brace_stack.pop()
                    if not brace_stack and start_pos != -1:
                        extracted = content[start_pos:i+1]
                        logger.debug(f"JSON提取成功: 括号匹配 (pos {start_pos}-{i})")
                        return extracted
                else:
                    # 不匹配，重置
                    brace_stack = []
